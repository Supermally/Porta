...
public class EngineService: ObservableObject {
    ...
    public func repairSteamSandbox() {
        let prefixPath = FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/MacGaming/launchers/steam"
        log("Initiating 1-Click Steam Sandbox Repair...", level: .info, source: "System")
        self.launchOutputMessage = "🛠️ Repairing Steam Sandbox... (Killing processes and purging corrupted cache)"
        ...
        var env = ProcessInfo.processInfo.environment
        env["WINEPREFIX"] = prefixPath
        killProc.environment = env
        killProc.arguments = ["wineserver", "-k"]
        try? killProc.run()
        killProc.waitUntilExit()
        
        let pkillProc = Process()
        pkillProc.executableURL = URL(fileURLWithPath: "/usr/bin/pkill")
        pkillProc.arguments = ["-9", "steamwebhelper"]
        try? pkillProc.run()
        ...
        if let runner = runnerPaths.first(where: { FileManager.default.fileExists(atPath: $0) }) {
            let regFilePath = prefixPath + "/drive_c/Program Files (x86)/Steam/dxvk_uninstall.reg"
            let regContent = """
            REGEDIT4

            [-HKEY_CURRENT_USER\\Software\\Wine\\AppDefaults\\steamwebhelper.exe\\DllOverrides]
            
            [-HKEY_CURRENT_USER\\Software\\Wine\\AppDefaults\\gldriverquery.exe\\DllOverrides]
            
            [-HKEY_CURRENT_USER\\Software\\Wine\\AppDefaults\\gldriverquery64.exe\\DllOverrides]
            """
            try? regContent.write(toFile: regFilePath, atomically: true, encoding: .utf8)
            
            let regProc = Process()
            regProc.executableURL = URL(fileURLWithPath: runner)
            var regEnv = ProcessInfo.processInfo.environment
            regEnv["WINEPREFIX"] = prefixPath
            regProc.environment = regEnv
            regProc.arguments = ["regedit", "C:\\Program Files (x86)\\Steam\\dxvk_uninstall.reg"]
            try? regProc.run()
            regProc.waitUntilExit()
        }

        // 2. Clear HTML and GPU caches
        let usersDir = prefixPath + "/drive_c/users"
        if let userList = try? FileManager.default.contentsOfDirectory(atPath: usersDir) {
            for user in userList {
                let htmlCache = usersDir + "/\(user)/AppData/Local/Steam/htmlcache"
        ...
        let steamDir = prefixPath + "/drive_c/Program Files (x86)/Steam"
        try? FileManager.default.removeItem(atPath: steamDir + "/appcache/httpcache")

        // 3. Clear existing user.reg so it can be regenerated by Wineserver or patched on next launch
        // We do not delete user.reg completely, but let the AppDefaults apply on next launch

        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            self.log("Steam Sandbox repair complete. Ready for clean launch.", level: .info, source: "System")
            self.launchOutputMessage = "✅ Steam Sandbox Repaired successfully! Caches cleared. Please launch Steam again."
        }
    }
}
