...
public class DependencyManager: NSObject, ObservableObject {
    ...
    override public init() {
        ...
        let base = appSupport.appendingPathComponent("Porta", isDirectory: true)
    ...
    public func installDependency(at index: Int) async throws {
        ...
        do {
            if dep.id == "runtime.wine-staging" {
                ...
                // Decompress tar.xz (asynchronous)
                let proc = Process()
                ...
                try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                    ...
                    proc.terminationHandler = { process in
                        if process.terminationStatus == 0 {
                            ...
                            continuation.resume(throwing: NSError(domain: "Porta.Extraction", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "Extraction exited with code \(process.terminationStatus)"]))
                        }
                    }
                ...
                if proc.terminationStatus != 0 {
                    throw NSError(domain: "Porta.Extraction", code: Int(proc.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "Extraction exited with code \(proc.terminationStatus)"])
                }
            ...
            self.dependencies[index].status = .failed(message: errorMsg, technicalDetails: error.localizedDescription)
            throw NSError(domain: "Porta.DependencyManager", code: 2, userInfo: [NSLocalizedDescriptionKey: errorMsg])
        }
        ...
        if FileManager.default.fileExists(atPath: dep.localPath) {
            ...
            throw NSError(domain: "Porta.DependencyManager", code: 3, userInfo: [NSLocalizedDescriptionKey: errorMsg])
        }
    }
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            ...
            let task = session.downloadTask(with: url) { tempURL, response, error in
                ...
                guard let tempURL = tempURL else {
                    continuation.resume(throwing: NSError(domain: "Porta.Download", code: 404, userInfo: [NSLocalizedDescriptionKey: "No data received."]))
                    return
                }
                do {
