...
public class PortaDiagnostics {
    public static let shared = PortaDiagnostics()
    
    private let logger = Logger(subsystem: "com.porta.runtime", category: "Diagnostics")
    ...
    public func captureCrashDump(pid: pid_t, reason: String) {
        ...
        task.arguments = ["\(pid)", "-file", "/tmp/porta_crash_\(pid).txt"]
        try? task.run()
    }
}
