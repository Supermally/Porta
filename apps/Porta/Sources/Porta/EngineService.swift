///
/// EngineService acts as the primary bridge between the Porta Swift frontend and the Forge Rust backend.
///
import AppKit
import Foundation
import SwiftUI
...
public class EngineService: ObservableObject {
    ...
    @Published public var enableMetalHud: Bool = UserDefaults.standard.bool(forKey: "Forge.enableMetalHud") {
        didSet {
            UserDefaults.standard.set(enableMetalHud, forKey: "Forge.enableMetalHud")
        }
    }
        ...
        let report = DiagnosticReportItem(
            ...
            findings: [
                ...
                DiagnosticFindingItem(
                    ...
                    severity: "info",
                    description: "Container verified at ~/Library/Application Support/Porta/prefixes/\(game.id). Registry and drive_c initialized.",
                    logSnippet: nil,
                    recommendedAction: "No action needed.",
                    autoFixCommand: nil
                ),
    ...
    public func detectSaveDirectory(for game: GameItem) -> String? {
        let prefixPath = FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/prefixes/\(game.id)"
        let steamPrefixPath = FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/launchers/steam"
    ...
    public func loadSaveManifest(for game: GameItem) -> GameSaveManifest {
        let saveVaultDir = FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/saves/\(game.id)"
    ...
    public var detectedWineRunnerPath: String {
        let runnerPaths = [
            (FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/Runtimes/Wine/Contents/Resources/wine/bin/wine"),
            (FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/Runner/Wine Staging.app/Contents/Resources/wine/bin/wine"),
...
"""
            ...
            if path.hasSuffix("BFRES.frag") && !content.contains("Porta View Mode Overrides:") {
...
"""
    ...
    public func launchGame(_ game: GameItem) {
        ...
        if let runner = detectedRunner, !game.executablePath.isEmpty && FileManager.default.fileExists(atPath: game.executablePath) {
            ...
            let shouldEnableHud = game.enableHud || enableMetalHud
            env["MTL_HUD_ENABLED"] = shouldEnableHud ? "1" : "0"
            env["DXVK_HUD"] = shouldEnableHud ? "devinfo,fps" : "0"
            env["WINEESYNC"] = game.enableEsync ? "1" : "0"
            env["WINEFSYNC"] = game.enableFsync ? "1" : "0"

            recordActivity(
                title: "Launched Game",
                details: "Started '\(game.title)' with \(game.useD3DMetal ? "D3DMetal" : "DXVK") (HUD: \(shouldEnableHud ? "ON" : "OFF")).",
                category: "Process",
                severity: .info
            )
            ...
            • Path: ~/Library/Application Support/Porta/prefixes/\(game.id)
    ...
    public func calibrateAllPrefixDisplays() {
        ...
        let prefixesDir = FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/prefixes"
    ...
    public func syncGogLibrary() {
        let gogSearchDirs = [
            ...
            "/Applications",
            FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/prefixes"
        ]
    ...
    public func openPrefixFolder(for game: GameItem) {
        let prefix = FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/prefixes/\(game.id)"
    ...
    public func stopGame() {
        ...
        let wineserverPaths = [
            ...
            (FileManager.default.homeDirectoryForCurrentUser.path + "/Library/Application Support/Porta/Runtimes/Wine/Contents/Resources/wine/bin/wineserver"),
