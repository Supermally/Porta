                ...
                settingsCard(title: "Liquid Glass Material & Optics", icon: "sparkles") {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        if engine.glassConfig.enabled {
                            ...
                            // Native Apple Glass Toggles
                            Toggle("High-Clarity Optical Glass (Clear Variant)", isOn: Binding(
                                get: { engine.glassConfig.variant == .clear },
                                set: { engine.glassConfig.variant = $0 ? .clear : .regular }
                            ))
                            .toggleStyle(.switch)

                            Toggle("Fluid Morphing Physics & Adaptive Geometry", isOn: Binding(
                                get: { engine.glassConfig.morphingMode == .full },
                                set: { engine.glassConfig.morphingMode = $0 ? .full : .off }
                            ))
                            .toggleStyle(.switch)

                            Toggle("Interactive Pointer & Hover Lens Refraction", isOn: Binding(
                                get: { engine.glassConfig.interactionResponse == .full },
                                set: { engine.glassConfig.interactionResponse = $0 ? .full : .off }
                            ))
                            .toggleStyle(.switch)

                            Divider().opacity(0.3)

                            // Native Apple Slider with Liquid Glass Styling
                            VStack(alignment: .leading, spacing: 8) {
                                HStack {
                                    Text("Liquid Glass Refraction Intensity")
                                        .font(.system(size: 13))
                                    Spacer()
                                ...
                                Slider(value: $testVolume, in: 0.0...1.0, step: 0.01)
                            ...
                            VStack(alignment: .leading, spacing: 12) {
                                ...
                                ZStack {
                                    ...
                                    HStack(spacing: 16) {
                                        MGPlayButton(state: .idle, onPlay: {})
                                        ...
                                        MGCompatibilityBadge(.native)
                                    }
                                    .padding(.horizontal, 14)
                                }
                                .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))

                                // Live Apple Slider Lens Test
                                VStack(alignment: .leading, spacing: 6) {
                                    HStack {
                                        Text("Native Slider Lens")
                                            .font(.caption)
                                            .foregroundColor(.secondary)
                                        Spacer()
                                        Text("\(Int(testVolume * 100))%")
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        HStack {
                            ...
                            Picker("", selection: $wineVersion) {
                                Text("Porta Managed Wine Staging (Default)").tag("Porta Managed Wine Staging (Default)")
                        ...
                        Toggle("Persist Shader Pre-Caching on Disk", isOn: $shaderCacheEnabled)
                            .toggleStyle(.switch)
                        Toggle("Metal Performance HUD Overlay", isOn: $engine.enableMetalHud)
                            .toggleStyle(.switch)
                    }
                }
